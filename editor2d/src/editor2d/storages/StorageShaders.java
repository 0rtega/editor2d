...
public class StorageShaders {
	...
	private Map<Figures, Shader> shaders;
	...
	public void cleanUp(){
		for(Map.Entry<Figures, Shader> pair: shaders.entrySet()){
	...
	public void initShaders() throws Exception{	
		...
		shaders.put(Figures.MILLHOLE, shader);
		...
		shader = createShader(res.getString("text3d(v)"), null, res.getString("text3d(f)"),
			...
			shaders.put(Figures.TEST_TEXT3D, shader);
	...
	public  Shader getShader(Figures figures){
